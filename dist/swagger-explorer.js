    ...
    exploreRoutePathAndMethod(instance, prototype, method, metatype, globalPrefix, modulePath, applicationConfig) {
        ...
        return (0, lodash_1.flatten)(allRoutePaths.map((routePath, index) => {
            ...
            if (requestMethod === common_1.RequestMethod.ALL) {
                ...
                return validMethods.map((requestMethod) => (Object.assign({ method: requestMethod.toLowerCase(), path: fullPath === '' ? '/' : fullPath, operationId: `${this.getOperationId(instance, method.name)}_${requestMethod.toLowerCase()}`, version: methodVersion || controllerVersion, versionType: applicationConfig.getVersioning() }, apiExtension)));
            }
            ...
            return Object.assign({ method: common_1.RequestMethod[requestMethod].toLowerCase(), path: fullPath === '' ? '/' : fullPath, operationId: this.getOperationId(instance, methodKey, pathVersion), version: methodVersion || controllerVersion, versionType: applicationConfig.getVersioning() }, apiExtension);
        }));
    }
